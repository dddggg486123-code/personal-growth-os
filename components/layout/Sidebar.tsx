...
const navItems = [
  ...
  { href: '/brain', label: '第二大脑', icon: Brain },
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-64 flex flex-col border-r transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border)' }}
      >
        <div className="flex items-center justify-between p-4 lg:p-6">
          <Link href="/" className="flex items-center gap-2" onClick={() => setSidebarOpen(false)}>
            <span className="text-xl">🌱</span>
            <span className="font-bold text-lg tracking-tight" style={{ color: 'var(--text-primary)' }}>Growth OS</span>
          </Link>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1 rounded-lg hover:bg-white/5">
        ...
        <div className="p-4 border-t space-y-3" style={{ borderColor: 'var(--border)' }}>
          <div className="flex items-center gap-3 px-3 py-2 rounded-lg" style={{ backgroundColor: 'var(--bg-card)' }}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
              style={{ backgroundColor: 'var(--accent)', color: '#fff' }}>{profile.level}</div>
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-1 h-1 rounded-full w-full" style={{ backgroundColor: 'var(--border)' }}>
                <motion.div className="h-1 rounded-full"
                  ...
                  initial={{ width: 0 }} animate={{ width: `${xpInfo.percent}%` }}
                  transition={{ duration: 0.8, ease: 'easeOut' }} />
              </div>
              <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{xpInfo.current} / {xpInfo.needed} XP</p>
            </div>
          </div>
